d3 = function() {
  ...
  function d3_geo_sphericalEqual(a, b) {
    return Math.abs(a[1] - b[1]) < ε && (Math.abs(Math.abs(a[1]) - π / 2) < ε || d3_geo_sphericalLongitudeDifference(a[0], b[0]) < ε);
  }
  ...
  function d3_geo_clipCircle(radius) {
    var cr = Math.cos(radius), point = [ radius, 0 ], smallRadius = cr > 0, point = [ radius, 0 ], notHemisphere = Math.abs(cr) > ε, interpolate = d3_geo_circleInterpolate(radius, 6 * d3_radians);
  ...
  function d3_geo_projectionMutator(projectAt) {
    ...
    }), k = 150, x = 480, y = 250, λ = 0, φ = 0, δλ = 0, δφ = 0, δγ = 0, δx, δy, preclip = d3_geo_clipAntimeridian, postclip = d3_identity, clipAngle = null, clipPolygon = null, clipExtent = null, stream;
